...
class GoogleMapsEvaluator:
    ...
    async def setup(self) -> None:
        ...
        #     model="llama3.1",                       # Change to another local model if desired
        #     temperature=0.0,
        # )
...
def read_queries(args: argparse.Namespace) -> List[str]:
    ...
    default_file = os.path.join(os.path.dirname(__file__), "maps_test_queries")
...
async def run_evaluation(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print("=" * 60)
        print(f"✅ Completed. Success: {succeeded}, Failures: {failed}. Results → {args.output}")
        return 0
    finally:
